...
function App() {
  ...
  function itemAddonClick(task) {
    ...
    setTask((prevVal) => [...prevVal,{ id: Date.now(), data: task, list: "OG", date: new Date().toLocaleString("en-IN")}]);
  }

  function doneTaskFun(id){ 
    setTask([...task.filter((element)=>(element.id!==id)),...task.filter((element)=>(element.id===id)&&(element.list="done"))])
  ...
  function restoreDroppedTask(id){
    setTask([...task.filter((element)=>(element.id!==id)),...task.filter((element)=>(element.id===id)&&(element.list="OG"))])
    ...
    <div className="App">
      ...
      <div className="listGroupDiv row">
        <div className="listDiv DoneItem">
          ...
          {(task.filter((elements)=>elements.list==="done")).map((element)=>
            <DoneItem 
            key={element.id}
            id ={element.id}
            data={element.data}
        ...
        <div className="listDiv OGItem">
          ...
          {(task.filter((elements)=>elements.list==="OG")).map((element) => (
            <OGItem
              id={element.id}
              key={element.id}
              data={element.data}
        ...
        <div className="listDiv DroppedItem">
          ...
          {(task.filter((elements)=>elements.list==="dropped")).map((element) => (
            <DroppedItem
              id={element.id}
              key={element.id}
              data={element.data}
